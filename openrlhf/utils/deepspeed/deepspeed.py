...
class DeepspeedStrategy(ABC):
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.args.zero_stage > 2 or self.args.ds_tensor_parallel_size > 1:
            try:
                # Try to use consolidated state dict for ZeRO-3
                if isinstance(model, Actor):
                    consolidated_dict = model.model._consolidated_16bit_state_dict()
                else:
                    consolidated_dict = model._consolidated_16bit_state_dict()
                
                # Check if consolidated dict is valid (not empty tensors)
                if consolidated_dict is not None and all(tensor.numel() > 0 for tensor in consolidated_dict.values() if hasattr(tensor, 'numel')):
                    output_state_dict = consolidated_dict
                else:
                    # Fallback to regular state dict if consolidated is invalid
                    from deepspeed.checkpoint.utils import clone_tensors_for_torch_save
                    output_state_dict = clone_tensors_for_torch_save(model_to_save.state_dict())
            except:
                # Silent fallback to regular state dict
                from deepspeed.checkpoint.utils import clone_tensors_for_torch_save
                output_state_dict = clone_tensors_for_torch_save(model_to_save.state_dict())
        else:
            from deepspeed.checkpoint.utils import clone_tensors_for_torch_save
